---
name: class-1-wrap-up
description: Activated when the student completes all steps of class 1. Invites the student to a review quiz and then routes to report submission.
compatibility: claude
---

## What I do

After a student finishes class 1, I invite them to participate in a review quiz
that reinforces the key concepts they just learned. I then conclude the session
and route them to report submission.

## When to use me

When the student completes all steps of class 1, or when a skill's final step
triggers it.
...
Present the quiz questions using the `question` tool, one section at a time.
...
> [!IMPORTANT]
>
> Only present the options listed for each question. Do not provide the correct
> answer or explanation text in the `question` tool call — that is for your own
> reference when reviewing responses afterwards.
...
Answer: 2 — `rpi-imager` needs block-level access to write raw image data to the
SD card, which requires superuser privileges.

||2 — Pre-imaging disk check||
...
Answer: 3 — `lsblk` lists block devices so you can confirm you are targeting the
SD card, not your computer's internal drive.

||3 — Headless pre-configuration||
...
the corresponding `board.D23` object at runtime, making the script reusable with
different pin assignments.

||11 — RuntimeError handling||
